const URL = {
    ...
    SEARCH: (title, type, page=false, season=false, episode=false) => {
        if(type == 'movie') {
            ...
            return `http://www.alluc.ee/stream/${title}+%23newlinks`;  
        }
        if( page != false ) return  `http://www.alluc.ee/stream/${title}+s${season}e${episode}+%23newlinks?page=${page}`;
        return  `http://www.alluc.ee/stream/${title}+s${season}e${episode}+%23newlinks`;
    }
};
...
class AllucHand {
    ...
    async getHostFromDetail() {
        ...
        for( let item of this.state.urlDetail ) {
            ...
            let htmlDetail = await httpRequest.getHTML(item);
            let $ = cheerio.load(htmlDetail); 
            let script = $('.linktitleurl script').html();
            let token  = script.match(/decrypt\(\'([^\']+)/i);
            ...
            let ccc = htmlDetail.match(/var *bbb *\= ([0-9]+)/i);
