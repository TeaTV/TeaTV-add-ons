...
var URL = {
    ...
    SEARCH: function SEARCH(title) {
        return 'http://www.icefilms.info/search.php?q=' + title + '&x=0&y=0';
    },
    ...
    LIST_SOURCE: function LIST_SOURCE(vid) {
        return 'http://www.icefilms.info/membersonly/components/com_iceplayer/video.php?h=374&w=631&vid=' + vid + '&img=';
    },
    GET_SOURCE_URL: function GET_SOURCE_URL(id, vid) {
        return 'http://www.icefilms.info/membersonly/components/com_iceplayer/video.php-link.php?s=' + id + '&t=' + vid;
    }
};
    ...
    _createClass(Icefilms, [{
        ...
        value: function () {
            var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
                var _libs, httpRequest, cheerio, stringHelper, base64, _movieInfo, title, year, season, episode, type, titleSearch, detailUrl, urlSearch, searchHtml, $;
                    ...
                    while (1) {
                        switch (_context.prev = _context.next) {
                            case 0:
                                ...
                                detailUrl = false;
                                urlSearch = URL.SEARCH(stringHelper.convertToSearchQueryString(titleSearch, '-'));
                                _context.next = 7;
                            ...
                            case 7:
                                ...
                                $('.SR tr').each(function () {
                                    var u = $(this).find('.title a').attr('href');
                                    var n = $(this).find('.title a').text();
                                    var s = n.split(/\s\(\d+\)/);
                                    var m = n.match(/\((\d+)\)/);
                                    var titleMovie = s[0].trim();
                                    var y = m != undefined ? m[1] : 0;
                                    if (stringHelper.shallowCompare(title, titleMovie) && !detailUrl) {
                                        if (type == 'movie' && year == y) detailUrl = u;else if (type == 'tv') detailUrl = u;
                                    }
                                });
                                            ...
                                            while (1) {
                                                switch (_context2.prev = _context2.next) {
                                                    case 0:
                                                        ...
                                                        headers['Referer'] = 'http://www.icefilms.info/membersonly/components/com_iceplayer/video.php?h=374&w=631&vid=' + videoId + '&img=';
                                                        headers['Cookie'] = cookie;
                                                        headers['Content-Type'] = 'application/x-www-form-urlencoded';
                                                        headers['Accept'] = 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8';
                                                        headers['Connection'] = 'keep-alive';
                                                        headers['Origin'] = 'http://www.icefilms.info';
